...
export type ResponseCreator = {
  ...
  error: (code: ErrorCode, error?: Error, options?: ResponseOptions) => ErrorResponse
  paginated: <T = any>(data: T[], page: number, pageSize: number, total: number, options?: ResponseOptions) => PaginatedResponse<T>
  list: <T = any>(data: T[], total: number, options?: ResponseOptions) => ListResponse<T>
  created: <T = any>(data: T, options?: ResponseOptions) => SuccessResponse<T>
  noContent: (options?: ResponseOptions) => SuccessResponse<null>
}

export type ApiResponse<T = any> = SuccessResponse<T> | ErrorResponse | PaginatedResponse<T> | ListResponse<T> | SuccessResponse<null>