...
export class Logger {
  ...
  private static validateConfig(): void {
    ...
    if (!this.config.maxSize || !/^(\d+)([bkmg]?)$/i.test(this.config.maxSize)) {
  ...
  static log(level: LogLevelEnum, message: string, context?: LogContext): void {
  ...
  static setLevel(level: LogLevelEnum): void {
    this.config.level = level
    this.getInstance().level = level
  }

  static getLevel(): LogLevelEnum {
    return this.config.level
  }

  static isLevelEnabled(level: LogLevelEnum): boolean {
  ...
  static logHttpRequest(method: string, url: string, status: number, duration: number, context?: LogContext): void {
    const level = status >= 400 ? LOG_LEVELS.WARN : LOG_LEVELS.INFO
    this.log(level, 'HTTP Request', { method, url, status, duration: `${duration}ms`, ...context })
  }
  ...
  static logAiRequest(model: string, prompt: string, duration: number, tokens?: { input: number; output: number }, context?: LogContext): void {
    this.info('AI Request', { model, promptLength: prompt.length, duration: `${duration}ms`, tokens, ...context })
  }
  ...
  static async clearLogsAsync(): Promise<void> {
    ...
    await Promise.all(
      files
        .filter(file => file.endsWith('.log'))
        .map(file => fs.promises.unlink(path.join(logDir, file)))
    )
  }
    ...
    files.forEach(file => {
      ...
      if (stats.mtime < cutoffDate) {
        try {
          ...
          deletedCount++
          this.debug(`删除过期日志文件: ${file}`)
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error)
          this.error(`删除过期日志文件失败: ${file}`, { error: errorMessage })
        }
      }
    })
   ...
   * @returns 删除的文件数量
   */
  static async cleanupOldLogsAsync(keepDays: number = DEFAULT_KEEP_DAYS): Promise<number> {
    const logDir = this.getLogDir()
    if (!fs.existsSync(logDir)) {
      return 0
    }
